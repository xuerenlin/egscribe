...
impl Store {
    ...
    pub fn open_file(&mut self, name: &str) -> std::io::Result<String> {
        ...
        // Fast binary check before decoding to avoid showing gibberish in editor.
        let raw_content = std::fs::read(name)?;
        if Self::is_likely_binary(&raw_content) {
            self.pending_non_text_file_prompt = Some(NonTextFilePrompt {
                file_path: name.to_string(),
                reason: "检测到文件包含较多二进制字节，可能不是纯文本文件。".to_string(),
            });
        ...
        // check new ctx
        if self.ectx_map.get(&curfile).is_none() {
            let text = if self.config.auto_detect_encoding {
                // Use encoding manager to read file
                match self.encoding_manager.read_file_as_utf8(name) {
                    Ok(content) => content,
                    Err(e) => {
                        log::error!("Failed to read file with encoding detection: {}", e);
                        // If encoding detection fails, try direct read
                        std::fs::read_to_string(name)?
                    }
                }
            } else {
                // Use default charset
                let default_charset = Charset::from_str(&self.config.default_charset);
                match self.encoding_manager.read_file_with_charset(name, &default_charset) {
                    Ok(content) => content,
                    Err(e) => {
                        log::error!("Failed to read file with default charset: {}", e);
                        // If failed, try direct read
                        std::fs::read_to_string(name)?
                    }
                }
            };

            let is_markdown = path_is_markdown_file(name);
        ...
        // Add to opened files order list
        // If file already exists, don't move position; if not, insert after current file
        if !self.opened_files.contains(&curfile) {
            let after = self.note_space.get_current_cur();
            self.opened_files.add(curfile.clone(), after.as_ref());
            // Update opened files list in config
    ...
    pub fn save(&mut self) -> std::io::Result<()> {
        if let Some(curfile) = self.note_space.get_current_cur() {
            ...
            if curfile.is_file() {
                ...
                // Check if it's an unsaved file (starts with "untitled/")
                if file_path.starts_with("untitled/") {
                    // Show file save dialog
                    if let Some(save_path) = self.show_save_file_dialog(&curfile.name()) {
                        // Save to new path
                        self.encoding_manager.write_file_with_encoding(&save_path, &text, &Charset::UTF8, &LineEnding::LF)?;
                        // Update file path
                        let new_file = UniFile::from(&save_path);
                        let old_file = curfile.clone();
                        // Create new Ctx (based on current text)
                    ...
                    let (charset, line_ending) = if let Some(encoding_info) = self.encoding_manager.get_file_encoding(&file_path) {
                    ...
                    match self.encoding_manager.write_file_with_encoding(&file_path, &text, charset, line_ending) {
                        Ok(_) => {},
                        Err(e) => {
                            log::error!("Failed to save file with encoding: {}", e);
                            // If encoding save fails, try direct save
                            self.note_space.write_file(&file_path, &text)?;
                        }
                    }
                    // Clear change flag
                    if let Some(ctx) = self.ectx_map.get_mut(&curfile) {
    ...
    /// Check and perform auto-save for notes if enabled and interval has passed
    /// Only saves notes (not files) and only if content has changed
    pub fn check_auto_save(&mut self) {
        ...
        // Auto-save all opened notes (not files)
        ...
        // Perform auto-save for all changed notes
        let mut has_saved = false;
        for note in notes_to_save {
            if let Err(e) = self.auto_save_note(&note) {
                log::error!("Auto-save failed for note {}: {}", note.name(), e);
            } else {
                has_saved = true;
            }
        }
        
        // Update last auto-save time if any note was saved
        if has_saved {
    ...
    /// Show file save dialog
    fn show_save_file_dialog(&self, file_name: &str) -> Option<String> {
        use rfd::FileDialog;
    ...
    pub fn execute_find(&mut self, find: FindReplaceCtx) {
        // Check if it's FindNotes command, if so, don't need to get result from current edit context
        let is_find_notes = if let Some(cmd) = &find.cmd {
            matches!(cmd, FindCmd::FindNotes)
        } else {
            false
        };
        ...
        // FindNotes has already set result in find_in_all_notes, no need to set again
        if is_find_notes {
            return;
        }
    ...
    pub fn execute_cmd(&mut self, cmd: Action) {
        match cmd.command.as_str() {
            ...
            // 设置行的扩展文本（用于插件显示计算结果等）
            "set_expanded_text" => {
                if let Ok(line_no) = cmd.get_number_param("line_no") {
                    let expanded_text = match cmd.get_optional_string_param("expanded_text") {
                        Ok(Some(text)) => Some(text),
                        Ok(None) => None,
                        Err(e) => {
                            log::warn!("Failed to get expanded_text parameter: {}", e);
                            None
                        }
                    };
                    
                    if let Some((_unifile, edit_ctx)) = self.cur_edit_ctx_mut() {
                        edit_ctx.set_expanded_text(line_no as usize, expanded_text);
                    } else {
                        log::warn!("No active editor context for set_expanded_text command");
                    }
                } else {
                    log::warn!("Failed to get 'line_no' parameter for set_expanded_text command");
                }
            }
            // 编辑器级别的动作不在这里处理，它们由编辑器上下文处理
            _ => {
                log::warn!("Unknown command: {}", cmd.command);
                // 忽略编辑器级别的动作（Copy, Cut, Paste等）
            }
        }
    }
