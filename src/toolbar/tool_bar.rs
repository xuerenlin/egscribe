...
impl ToolBar<'_> {
    fn with_transparent_menu_button<R>(ui: &mut Ui, add_contents: impl FnOnce(&mut Ui) -> R) -> R {
        ui.scope(|ui| {
    ...
    fn tool_bar(store: &mut Store, ui: &mut Ui) {
        ...
        // language switcher
        Self::with_transparent_menu_button(ui, |ui| {
        ...
        Self::with_transparent_menu_button(ui, |ui| {
            ui.menu_button(
                ...
                if ui
                    .selectable_label(
                        store.config.show_table_head_checkbox,
                        tr("toolbar.table_head_checkbox.toggle"),
                    )
                    .clicked()
                {
                    store.config_switch_table_head_checkbox();
                    ui.close();
                }
            });
        });
